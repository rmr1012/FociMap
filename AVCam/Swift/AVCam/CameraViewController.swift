...
var count = 0
var sessionId = 0
let sizes : [Float] = [0.6, 0.65, 0.70, 0.72, 0.75, 0.78, 0.8, 0.95]
//let mainUrl: String = "http://10.10.4.112:3000/"
let mainUrl: String = "http://35.237.125.164:80/"
var sessionStatus = [Int : Int]()

class CameraViewController: UIViewController, AVCaptureFileOutputRecordingDelegate, SFSafariViewControllerDelegate {
...
//            }
            ...
            //var currentLensPosition : Float = 0.4
            for size in sizes {
                ...
                do{
                    ...
                    if let backCameraDevice = AVCaptureDevice.default(.builtInWideAngleCamera, for: .video, position: .back) {
                        // default to the back wide angle camera.
                        defaultVideoDevice = backCameraDevice
                    } else if let frontCameraDevice = AVCaptureDevice.default(.builtInWideAngleCamera, for: .video, position: .front) {
                        /*
                         In some cases where users break their phones, the back wide angle camera is not available.
                    ...
                    do {
                        ...
                        device.setFocusModeLocked(lensPosition: size)
                        device.unlockForConfiguration()
                    } catch {
                        print("Could not lock device for configuration: \(error)")
                    }
                ...
                usleep(500000)
                let newSettings: AVCapturePhotoSettings = AVCapturePhotoSettings.init(from: photoSettings)
                self.photoOutput.capturePhoto(with: newSettings, delegate: photoCaptureProcessor)
                //currentLensPosition += stepSize
                
            }
            while(sessionStatus[sessionId]! < sizes.count) {
                print(sessionStatus[sessionId]!)
                sleep(1)
            }
//            struct thePairs {
//                var filename: String
...
//            struct json {
//                var id : Int
//                var fileset : [thePairs]
//            }
//
//            var myPairs: [thePairs] = []
//            for i in 0...(sizes.count-1) {
//                myPairs.append(thePairs(filename: "\(sessionId)_\(i).jpg", focal: sizes[i]))
//            }
//            let myJson = json(id: sessionId, fileset: myPairs)
////
//            var twodDict: [String : Any] = [:]
//            twodDict["id"] = sessionId
//            var temp: [[String : Any]] = []
//            for i in 0...(sizes.count-1) {
//                print("HELLO")
//                var aba: [String : Any] = [:]
//                aba["filename"] = "\(sessionId)_\(i).jpg"
//                aba["focal"] = sizes[i]
//                //var temp = twodDict["fileset"] as! [[String : Any]]
//                temp.append(aba)
//            }
//            twodDict["fileset"] = temp
//            let params = twodDict
            
            //let encoder = JSONEncoder()
            //encoder.outputFormatting = .prettyPrinted
            //print(String(data: try encoder.encode(myJson), encoding: .utf8)!)
            let params = ["id": sessionId]
            HTTP.POST(mainUrl+"url", parameters: params) { response in
                if let err = response.error {
                    print("error: \(err.localizedDescription)")
                } else {
                    //print(response.text)
                    var sessionIdStr = String(sessionId)
                    var sessionURL = mainUrl+sessionIdStr
                    let svc = SFSafariViewController(url: URL(string: sessionURL )!)
                    svc.delegate = self
